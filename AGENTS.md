...
2. `Resources/Info.plist` `CFBundleShortVersionString` and `CFBundleVersion`,
   both the same string (the About panel shows one number because
   `AppDelegate.showAbout` passes an empty build string; AppKit would
   otherwise print CFBundleVersion in parentheses, equal or not);
